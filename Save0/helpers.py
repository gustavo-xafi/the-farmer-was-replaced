...
def clean_harvest_and_till(harvestable, crop, matrix):
    ...
    if crop == Entities.Tree and tree_rule() == True:
        ...
        crop = Variables.crop_list_without_tree[
            random() * len(Variables.crop_list_without_tree) // 1
        ]
...
def treasure_catch_and_reuse():
    if get_entity_type() == Entities.Treasure:
        return create_maze()

def treasure_catch_and_restart():
    if get_entity_type() == Entities.Treasure:
        harvest()
        return create_maze()
